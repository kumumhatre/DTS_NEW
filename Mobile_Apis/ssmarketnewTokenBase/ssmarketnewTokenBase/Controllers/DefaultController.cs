...
namespace ssmarketnewTokenBase.Controllers
{
    ...
    public class DefaultController : ApiController
    {
        ...
        [Route("api/Default/Userinfo")]
        [HttpGet]
        public HttpResponseMessage Userinfo(string clientCode)
        {
            Utils _utils = new Utils();
            SqlConnection FeedConn = Utils.feedconn;
            SqlConnection Conn = Utils.conn;
            ...
            //{
                ...
                Limitset objlimits = new Limitset();
                ...
                BrkgType _data = _utils.DownloadBrkgType(clientCode);
                ...
                Decimal netmtm = 0, realisedpl = 0, unrealisedpl = 0,Brkg = 0;
                Dictionary<string, buysellnetpospfls> _NetMTMProftLoss = new Dictionary<string, buysellnetpospfls>();
                ...
                foreach (var items in _NetPosPfLs)
                {
                    ...
                    int _Buysell = items.Value.buy_sell;
                    if (_Buysell == 1)
                        Buysell = "BUY";
                    else
                        Buysell = "SELL";
                    ...
                    unrealisedpl += mtm;
                    objnetposs.TurnoverUtilised += objpos.TurnoverUtilised;
                    cnt++;
                    Brkg += items.Value.Commision;
                
                }
                objlimits.netmtm += realisedpl + unrealisedpl - Brkg;
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                if (!reader.IsDBNull(33))
                                    objlimits.PendingOrderExec = reader.GetInt32(33);

                                objlimits.brkupType = !reader.IsDBNull(35) ? reader.GetInt32(35) : 2;

                                if (cnt != 0)
                                    objlimits.unrealisedpl = unrealisedpl;
                                else
                                    objlimits.unrealisedpl = 00;
                                if (cnt != 0)
                                    objlimits.utilizedMargin = objnetposs.TurnoverUtilised;
                                else
                                    objlimits.utilizedMargin = 00;
                                objlimits.realisedpl = realisedpl;
                                objlimits.Brokrage = Brkg;
                                objlimits.brkg_mcx = _data.mcx;
                                objlimits.brkg_NseFut = _data.nsefut;
                                objlimits.brkg_NseCurr = _data.nsecurr;
                                objlimits.brkg_Ncdex = _data.ncdex;
                ...
                //li.Add(new Limits { clientcode=objlimits.clientcode, cashmrgn=objlimits.cashmrgn, turnoverlimit =objlimits.turnoverlimit, mtmlosslimit =objlimits.mtmlosslimit, turnmulti =objlimits.turnmulti, mtmmulti =objlimits.mtmmulti ,});
                JsonSerializerSettings settings = new JsonSerializerSettings();
                settings.Formatting = Formatting.Indented;
                settings.ContractResolver = new DictionaryAsArrayResolver();
                var json1 = JsonConvert.SerializeObject(objlimits, settings);
                json1 = "{\"data\":" + json1 + "}";
