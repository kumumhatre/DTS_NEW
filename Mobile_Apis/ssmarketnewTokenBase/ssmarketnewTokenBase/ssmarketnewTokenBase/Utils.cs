...
namespace ssmarketnewTokenBase
{
    public class Utils
    {
        ...
        public static SqlConnection feedconn = new SqlConnection(@"Data Source=207.180.255.236\\M11836\\SQLEXPRESS,49170;User ID=jerry;Initial Catalog=DTS;Password=Jerry@((764));Integrated Security=false;MultipleActiveResultSets = True;Connect Timeout=500000000");
        public static SqlConnection ohlcconn = new SqlConnection(@"Data Source=207.180.255.236\\M11836\\SQLEXPRESS,49170;User ID=jerry;Initial Catalog=OHLCData;Password=Jerry@((764));Integrated Security=false;MultipleActiveResultSets = True;Connect Timeout=500000000");
        public static SqlConnection fivestarconn = new SqlConnection(@"Data Source=207.180.255.236\\M11836,1433;User ID=tom;Initial Catalog=5starREP;Password=Tom@(764);Integrated Security=false;MultipleActiveResultSets = true;Connect Timeout=500000000");
        public static SqlConnection oldfeedconn = new SqlConnection(@"Data Source=164.68.123.204\\M13504,1433;User ID=sa;Initial Catalog=DTS;Password=aFFWL@786;Integrated Security=false;MultipleActiveResultSets = True;Connect Timeout=500000000");
        public static SqlConnection oldohlcconn = new SqlConnection(@"Data Source=164.68.123.204\\M13504,1433;User ID=sa;Initial Catalog=OHLCData;Password=aFFWL@786;Integrated Security=false;MultipleActiveResultSets = True;Connect Timeout=500000000");
        public static SqlConnection conn1 = new SqlConnection(@"Data Source=164.68.123.204\\M13504,1433;User ID=sa;Initial Catalog=DTS1;Password=aFFWL@786;Integrated Security=false;MultipleActiveResultSets = True;Connect Timeout=500000000");
            ...
            foreach (var items in _NetPosPfLs)
            {
                ...
                if (objnetpos.symbol == objtrd.Symbol)
                {
                    if (objnetpos.BQty > objnetpos.SQty)
                    {
                        ...
                        if (/*openqty >= objtrd.qty && */objtrd.buysell == 2 && validity == objtrd.validity)
                            isclosepos = true;
                        else
                            isclosepos = false;
                    }
                    else if (objnetpos.BQty < objnetpos.SQty)
                    {
                        ...
                        if (objcon.exch == 2 /*|| objcon.exch == 5*/ && _info.oddlot == 0)
                            ...
                            SymLots += openqty;
                        if (/*openqty >= objtrd.qty &&*/ objtrd.buysell == 1 && validity==objtrd.validity)
                            isclosepos = true;
                        else
                            isclosepos = false;
                    }
        ...
        public decimal GetBegningBal(string regucode, DateTime startdate, string enddate)
        {
            ...
            if (conn.State == ConnectionState.Open)
            {
                ...
                using (var cmd = new SqlCommand("GetBegningBal", conn) { CommandType = CommandType.StoredProcedure })
                {
                    ...
                    cmd.Parameters.Add(new SqlParameter("@fromdate", startdt.ToString("yyyy-MM-dd 09:00:00")));
                    cmd.Parameters.Add(new SqlParameter("@todate", enddate));
