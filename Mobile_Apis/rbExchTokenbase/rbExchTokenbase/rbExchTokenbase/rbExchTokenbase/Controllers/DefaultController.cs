...
namespace rbExchTokenbase.Controllers
{
    ...
    public class DefaultController : ApiController
    {
        ...
        public HttpResponseMessage setAlert([FromBody]Alert al)
        {
            ...
            else
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SaveAlerts", FeedConn) { CommandType = CommandType.StoredProcedure })
                {
                    ...
                    cmd.Parameters.AddWithValue("@condition", al.condition);
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            if (status.Equals("0"))
                                ...
                                timestamp = Convert.ToDateTime(reader.GetValue(5));
                            li.Add(new Alert { id = tp.id, symbol = tp.symbol, condition = tp.condition, price = tp.price, timestamp = timestamp.ToString("yyyy-MM-dd HH:mm:ss.fff") });
                        }
        ...
        [Route("api/Default/getmsg")]
        [HttpGet]
        public HttpResponseMessage getmsg(string clientcode)
        {
            Utils _utils = new Utils();
            SqlConnection Conn = Utils.conn;
            FRatesResponse response = new FRatesResponse();
            ...
            else
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[lowertouperHirarchy]", Conn) { CommandType = CommandType.StoredProcedure })
                {
                    cmd.Parameters.AddWithValue("@Clientcode", clientcode);
                ...
                using (SqlCommand cmd = new SqlCommand("[getStatusMsgForMobile]", Conn) { CommandType = CommandType.StoredProcedure })
                {
